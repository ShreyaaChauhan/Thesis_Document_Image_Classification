"""
If a function gets defined once and could be used over and over,
...
# and find out how many files (images)
...
def walk_through_dir(dir_path):
    ...
    for dirpath, dirnames, filenames in os.walk(dir_path):
        print(
            f"There are {len(dirnames)} directories \
...
def pred_and_plot_image(
    ...
    with a trained model and plots the image.

    Args:
        model (torch.nn.Module): trained
        PyTorch image classification model.
        image_path (str): filepath to target image.
        class_names (List[str], optional): different class names
        for target image. Defaults to None.
        transform (_type_, optional): transform of target image.
        Defaults to None.
    ...
    if class_names:
        title = f'Pred: {class_names[target_image_pred_label.cpu()]} | Prob: {target_image_pred_probs.max().cpu():.3f}'  # noqa
    else:
        title = f'Pred: {target_image_pred_label} | Prob: {target_image_pred_probs.max().cpu():.3f}'  # noqa
    plt.title(title)
    plt.axis(False)
...
def download_data(
    ...
    Args:
        ...
        remove_source (bool): Whether to remove the source after
